...
def depth(snailNumber): #Returns maximum depth of tree
    if type(snailNumber)==int: return 1
...
def split(snailNumber): #Splits the tree
    if type(snailNumber) == int: return
...
def explode(snailNumber, snailNumberWhole, depth = 1, itemsSoFar = 0): #Explodes the tree
    ...
    if type(snailNumber[0]) == list:
        if depth >= 4 and type(snailNumber[0][0]) == int and type(snailNumber[0][1]) == int:
            if itemsSoFar < treeSize(snailNumberWhole) - 1: setNthItem(snailNumberWhole, itemsSoFar + 2, findNthItem(snailNumberWhole, itemsSoFar + 2) + snailNumber[0][1])
            if itemsSoFar > 0: setNthItem(snailNumberWhole, itemsSoFar - 1, findNthItem(snailNumberWhole, itemsSoFar - 1) + snailNumber[0][0])
            snailNumber[0] = 0
            return True
        else:
            result = explode(snailNumber[0], snailNumberWhole, depth + 1, itemsSoFar)
            if result: return True
    
    if type(snailNumber[1]) == list:
        if depth >= 4 and type(snailNumber[1][0]) == int and type(snailNumber[1][1]) == int:
            if itemsSoFar < treeSize(snailNumberWhole) - 1: setNthItem(snailNumberWhole, itemsSoFar + 3, findNthItem(snailNumberWhole, itemsSoFar + 3) + snailNumber[1][1])
            if itemsSoFar >= 0: setNthItem(snailNumberWhole, itemsSoFar, findNthItem(snailNumberWhole, itemsSoFar) + snailNumber[1][0])
            snailNumber[1] = 0
            return True
        else:
            result = explode(snailNumber[1], snailNumberWhole, depth + 1, itemsSoFar + treeSize(snailNumber[0]))
            if result: return True
    return False
...
print(f"The answer is {magnitude(numbers[0])}")
