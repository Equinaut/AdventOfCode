...
messageDelay = 10
print(f"Restarted program at {time.strftime('%X')} ({int(time.time())})\n\n")
...
def allPossibleStates(energy, state):
    ...
    for type, x in state["hall"]:  # Hall to bucket
        ...
        if len(state["buckets"][homeBucket]) >= 4:
            continue
    ...
    for i, bucket in enumerate(state["buckets"]):  # Bucket to hall
        if len(bucket) == 0:
            continue
        correctType = "ABCD"[i]
        if all([j == correctType for j in bucket]):
            continue
        x = i*2 + 3
...
def finished(state):
    ...
    for i, bucket in enumerate(state["buckets"]):
        if len(bucket) != 4:
            return False
        correctItem = "ABCD"[i]
        if any([item != correctItem for item in bucket]):
            return False
    return True
...
def addToQueue(newEnergyWithH, newEnergy, steps, state):
    ...
    while newEnergyWithH > queue[i][0] and i < len(queue):
        i += 1
...
def heuristic(state):
    ...
    for i, bucket in enumerate(state["buckets"]):
        ...
        for k, j in enumerate(bucket):
            ...
            if bucketX != homeX:
                value += (abs(homeX - bucketX)) * \
                    {"A": 1, "B": 10, "C": 100, "D": 1000}[j]
                value += (5-(len(bucket)-k)) * \
                    {"A": 1, "B": 10, "C": 100, "D": 1000}[j]
    return value
...
while len(queue) > 0:
    ...
    if hashState(state) in previouslyDone:
        continue
    previouslyDone.add(hashState(state))

    if time.perf_counter() - lastLogged > messageDelay:
        print(f"Current queue length: {len(queue)}\nCurrent energy: {energy}\nCurrent energyWithH: {energyWithH}\nCurrent time: {time.strftime('%X')} ({int(time.time())})\n")
        lastLogged = time.perf_counter()
