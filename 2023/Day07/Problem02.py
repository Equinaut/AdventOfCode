def main():
    ...
    with open("input.txt") as file:
        ...
        for line in file:
            ...
            _, rank, hand = getMaxRank(hand, hand)
            ranks[rank].append((hand, score))
    j = 1
    for r in ranks:
...
def getMaxRank(hand, originalHand):
    cardVals = "23456789TQKA"
    if "J" in hand:
        newHand = list(hand)
        answers = []
        for newCard in cardVals:
            newHand[hand.index("J")] = newCard
            newRank = getMaxRank("".join(newHand), originalHand)
            answers.append((newRank[0], newRank[1], originalHand))
        return max(answers, key = lambda k: k[1])
    else:
        ...
        return hand, rank, originalHand

def handValue(hand):
    values = {"T": 10, "J": 0, "Q": 12, "K": 13, "A": 14}
