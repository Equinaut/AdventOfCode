...
def transform(scanner0, scanner1, scanner0Scanner0Offset, scanner1Scanner0Offset, flips, orientation):
    for coord in getCoords(scanner1, flips, orientation, scanner1Scanner0Offset):
        ...
        if scanner0 not in coordsRelativeTo:
            coordsRelativeTo[scanner0] = set()
...
for scanner in scanners:
    if scanner not in coordsRelativeTo:
        coordsRelativeTo[scanner] = set()
    for beacon in scanner.beacons:
        coordsRelativeTo[scanner].add(beacon)

for scanner0 in scanners:
    for scanner1 in scanners:
        if scanner0.name == scanner1.name:
            continue
                    ...
                    for flips2 in range(-1, 2, 2):
                        ...
                        for beacon in scanner0.beacons:
                            for beacon2 in scanner1.beacons:
                                scanner0Scanner0Offset = (
                                    beacon[0] * -1, beacon[1] * -1, beacon[2] * -1)
                                scanner1Scanner0Offset = (
                                    beacon2[orientation[0]] * flips[0] * -1,
                                    beacon2[orientation[1]] * flips[1] * -1,
                                    beacon2[orientation[2]] * flips[2] * -1)

                                scanner0Offsets = getCoords(
                                    scanner0, offsetFromScanner0=scanner0Scanner0Offset)
                                scanner1Offsets = getCoords(
                                    scanner1, flips, orientation, scanner1Scanner0Offset)
                                intersection = scanner0Offsets.intersection(
                                    scanner1Offsets)
                                ...
                                if len(intersection) >= 12:
                                    transformations.append(
                                        (scanner0, scanner1, scanner0Scanner0Offset, scanner1Scanner0Offset, flips, orientation))

                                    print(
                                        f"{actualIntersection}, \t,  {flips},{orientation}, \t{len(intersection)}, \t{scanner1Scanner0Offset}")
                                    print()
                                    print(
                                        f"All {scanner1.name} coords relative to {scanner0.name}")
...
scannerPositions = {}
for scanner in scanners:
    for flipX in range(-1, 2, 2):
        for flipY in range(-1, 2, 2):
            for flipZ in range(-1, 2, 2):
                flips = (flipX, flipY, flipZ)
                for orientation in itertools.permutations((0, 1, 2)):
                    relativeCoords = scanner.getCoordinates(flips = flips, axis = orientation)
                    for startCoord in allCoords:
                        guessedScannerPosition = (
                            startCoord[0] - list(relativeCoords)[0][0],
                            startCoord[1] - list(relativeCoords)[0][1],
                            startCoord[2] - list(relativeCoords)[0][2]
                        )
                        newRelativeCoords = set()
                        for relativeCoord in relativeCoords:
                            newRelativeCoords.add((relativeCoord[0] + guessedScannerPosition[0], relativeCoord[1] + guessedScannerPosition[1], relativeCoord[2] + guessedScannerPosition[2]))
                        if all([coord in allCoords for coord in newRelativeCoords]):
                            scannerPositions[scanner] = guessedScannerPosition
...
for scanner1 in scanners:
    for scanner2 in scanners:
        newDistance = manhattanDistance(scannerPositions[scanner1], scannerPositions[scanner2])
        if newDistance > answer:
            answer = newDistance
